...
const Impacto = () => {
  ...
  useEffect(() => {
    if (!isDesktop || !shouldLoadSpline || !percentageRef.current || !splineContainerRef.current) return;
    ...
    const updateSplinePosition = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const percentageCenterX = percentageRect.left + percentageRect.width / 2 - sectionRect.left;
        const percentageCenterY = percentageRect.top + percentageRect.height / 2 - sectionRect.top;
  ...
  const updateCard = useCallback(
    (latest) => {
      const cardIndex = Math.min(Math.floor(latest * cards.length), cards.length - 1);
  ...
  return (
    <div ref={containerRef} className="relative" style={{ height: `${cards.length * 80}vh` }} data-dark-section="true">
      <section ref={sectionRef} className="sticky top-0 bg-grey-40 overflow-hidden h-screen px-[16px] md:px-[146px] lg:px-[128px] xxl:px-[256px] py-[64px] flex flex-col items-center justify-center">
        ...
        <div className="relative h-[400px] md:h-[547px] w-full lg:flex lg:items-center lg:h-auto lg:min-h-[400px] lg:gap-[64px] lg:justify-center lg:w-[870px] z-10">
          ...
          <motion.div
            ...
            <div className="w-[250px] h-[250px] md:w-[350px] md:h-[350px] aspect-square flex-shrink-0 rounded-full overflow-hidden">
              <video className="w-full h-full object-cover pointer-events-none" autoPlay loop muted playsInline preload="auto">
                <source src="/videos/orb-full.webm" type="video/webm" />
                Your browser does not support the video tag.
              </video>
            </div>
          </motion.div>
          <div ref={percentageRef} className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl lg:relative lg:translate-y-0 lg:translate-x-0 lg:top-0 lg:left-0 lg:w-[470px]">
            ...
            <motion.div
              ...
              <div className="w-[420px] h-[420px] min-w-[420px] min-h-[420px] aspect-square flex-shrink-0 rounded-full overflow-hidden">
                <video className="w-full h-full object-cover scale-100" autoPlay loop muted playsInline preload="auto">
                  <source src="/videos/orb-full.webm" type="video/webm" />
                  Your browser does not support the video tag.
                </video>
              </div>
            ...
            <div className="relative w-full md:h-[230px] pb-[55px] md:pb-0 md:mb-[128px] flex items-center justify-center lg:mb-0 z-10 overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  {cards[currentCard].symbol && <p className="text-grey-00 text-[36px]! leading-[110%]! tracking-[-0.72px]!">{cards[currentCard].symbol}</p>}
                  {cards[currentCard].percentage && (
                    <div className="flex items-center gap-2">
                      <p className="text-grey-00 font-sora! text-[96px]! leading-[110%]! tracking-[-0.96px]!">
                        <DelayedNumberFlow value={cards[currentCard].percentage} delay={500} />
                      </p>
                      <p className="text-grey-00 text-[36px]! leading-[110%]! tracking-[-0.72px]!">%</p>
                    </div>
                  )}
                </motion.div>
              </AnimatePresence>
          ...
          <div className="absolute lg:relative top-[200px] md:top-auto md:bottom-0 left-1/2 -translate-x-1/2 w-full max-w-4xl lg:w-[380px] lg:max-w-full lg:translate-y-0 lg:translate-x-0 lg:top-0 lg:left-0">
            <div className="lg:flex lg:flex-col lg:items-start lg:justify-center lg:min-h-[280px]">
              <AnimatePresence mode="wait">
                <motion.div key={currentCard} initial="hidden" animate="visible" exit="exit" className="lg:flex lg:flex-col lg:items-start will-change-transform">
                  ...
                  <motion.div
                    ...
                    <p className="text-grey-00 pb-[16px] text-xl font-semibold xxl:w-[550px]">{cards[currentCard].desc1}</p>
                    <p className="text-grey-00 text-lg xxl:w-[550px]">{cards[currentCard].desc2}</p>
                  </motion.div>
                </motion.div>
              </AnimatePresence>
            </div>
